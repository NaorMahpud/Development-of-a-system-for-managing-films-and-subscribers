...
export default function MenuPage() {
    ...
    return (
        <div>
            <h1 style={{ paddingRight: '160px', fontFamily: 'Ariel', justifyContent: 'center', display: 'flex', color: 'blue' }}>
                Hello {sessionStorage.getItem('fullName')}
            </h1>
            <div style={{ display: 'flex', justifyContent: 'space-evenly' }}>

                <Link to={'./movies/allmovies'}>
                    <Button disabled={!canViewMovies} style={getButtonStyle(1)} onClick={() => handleNavigation('./movies/allmovies', 1)}>
                        Movies
                    </Button>
                </Link>

                <Link to={'./subscriptions'}>
                    <Button disabled={!canViewSubscriptions} style={getButtonStyle(2)} onClick={() => handleNavigation('./subscriptions', 2)}>
                        Subscriptions
                    </Button>
                </Link>
                ...
                {role === 'Admin' && (
                    <Link to={'./users/allusers'}>
                        <Button style={getButtonStyle(3)} onClick={() => handleNavigation('./users/allusers', 3)}>
                            Users Management
                        </Button>
                    </Link>
                )}
